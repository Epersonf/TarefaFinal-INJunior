...
router.post('/register', function (req, res) {
  User.register(new User(req.body), req.body.password, function (err, user) {
    ...
    passport.authenticate('local')(req, res, function () {
      return res
        .status(200)
        .json({ status: 'Usuário registrado', id: user._id });
    });
  });
});
...
router.get('/consultores/num', function (req, res, next) {
  User.find({ tipo: 'Consultor', status: 'Aprovado' }).count(function (
    err,
    user
  ) {
    if (err) throw err;
...
router.get('/consultores/:supervisorId', function (req, res, next) {
  //por supervisor
  User.find({ tipo: 'Consultor', supervisor: req.params.supervisorId })
    .populate({
      path: 'supervisor',
      select: 'nome _id'
    })
...
router.get('/consultores/indicados/:indicadorId', function (req, res, next) {
  //por indicador
  User.find({ indicador: req.params.indicadorId })
    .populate({
      path: 'supervisor',
      select: 'nome _id'
    })
...
router.post('/consultores/maisdesconto', function (req, res, next) {
  User.update({ _id: req.body._id }, { $inc: req.body.inc }, function (
    err,
    consultor
  ) {
    if (err) throw err;
...
router.get('/estoqueId/', function (req, res, next) {
  //por supervisor
...
router.post('/atualizar', function (req, res, next) {
  User.update({ _id: req.body._id }, { $set: req.body.update }, function (
    err,
    consultor
  ) {
    if (err) throw err;
...
router.post('/entrada', function (req, res, next) {
  User.update(
    { _id: req.body._id },
    { $push: { estoque: { $each: req.body.entrada } } },
...
router.post('/estoque/:id', function (req, res, next) {
  User.update(
    { _id: req.params.id },
    { $push: { estoque: { $each: req.body.estoque } } },
    function (err, consultor) {
      if (err) throw err;
      res.json('Peças adicionadas!');
    }
...
router.post('/vendido/:id', function (req, res, next) {
  User.update(
    { _id: req.params.id },
    { $push: { estoque: { $each: req.body.vendido } } },
    function (err, consultor) {
      if (err) throw err;
      res.json('Peças adicionadas!');
    }
...
router.post('/venda', function (req, res, next) {
  User.update({ _id: req.body._id }, { $set: req.body.update }, function (
    ...
    if (err) throw err;
    PecaLog.create(
      { acao: 'venda', pecas: req.body.pecas, user: req.body.userId },
      function (err, pecalog) {
        if (err) throw err;
...
router.post('/estorno', function (req, res, next) {
  User.update({ _id: req.body._id }, { $set: req.body.update }, function (
    ...
    if (err) throw err;
    PecaLog.create(
      { acao: 'estorno', pecas: req.body.pecas, user: req.body.userId },
      function (err, pecalog) {
        if (err) throw err;
